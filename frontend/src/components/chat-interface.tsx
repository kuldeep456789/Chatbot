...
// Empty State with Writing Prompts + Input
        ...
        <div className="h-full flex items-center justify-center">
          <div className="text-center px-4">
            ...
            <p className="text-sm text-muted-foreground">
              Start the conversation and let's create something amazing
              together.
            </p>
          </div>
        </div>
...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({
  ...
  const ChannelMessageInputComponent = () => {
    const { sendMessage: rawSendMessage } = useChannelActionContext();
    ...
    const handleSendMessage: ChatInputProps["sendMessage"] = async (message) => {
  if (!channel || !message.text.trim()) return;

  setIsGenerating(true);
  try {
    await rawSendMessage({
      message: { text: message.text },   // ✅ wrapped inside message
      options: { waitForServer: true },  // ✅ proper options field
    });
    setInputText("");
  } catch (err) {
  ...
  const channelName =
    typeof (channel?.data as any)?.name === "string"
      ? (channel?.data as any).name
      : channel?.data?.id ?? "New Writing Session";
